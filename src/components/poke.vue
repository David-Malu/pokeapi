    ...
    <div class="container-fluid" style="font-family:'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;">
      <div class="row">
        ...
        <div class="col-sm-3  " style="margin-top: 40px; ">
          <select class="form-select" aria-label="Default select example" v-model="tipoSeleccionado" @change="filtrarPorTipo">
            <option value="">Todos los tipos</option>
            <option v-for="tipo in tiposPokemon" :key="tipo" :value="tipo">{{ tipo }}</option>
          </select>
        </div>
        <div class="col-sm-4" id="titulo">
          <h1>PokeApi</h1>
        </div>

        <div class="col-sm-3">

          <div class="input-group mb-3" style="margin-top: 40px; width: 200px;">
            <input @input="buscarPokemon" v-model="pokemonBuscado" type="text" class="form-control"
      ...
      <div class="row">
        <div class="col-md-4 " v-for="pokemon in pokemonLista" :key="pokemon" style="width: 300px;">
          <div class="card ">
            <h6> <b>
                N°{{ pokemon.id }}
              </b> </h6>
            <h6>Tipo: <b style="color: blue;">{{ pokemon.type }}</b></h6>
            <img :src="pokemon.image" class="card-img-top " :alt="pokemon.name" />
            <div class="card-body ">
              <div class="row ">
                <div class="col-sm-2"></div>
                <div class="col-sm-8" id="centrar">

                  <h4 class="card-title"> {{ pokemon.name }}</h4>
                </div>
                <div class="col-sm-2"></div>
              </div>
              <div class="row border-top border border-3 "></div>
              <div class="row mt-3">
                <div class="col-sm-2 "></div>
                <div class="col-sm-8" id="centrar">
                  <div class="btn-group" role="group" style="">
                    ...
                    <router-link
                      :to="`/galeria/${pokemon.name}/${encodeURIComponent(pokemon.image)}/${pokemon.power}/${pokemon.id}/
                                                                ${pokemon.peso}/${pokemon.hp}/${pokemon.defense}/
                                                                ${pokemon.attack}/${pokemon.especialAttack}/
                                                                ${pokemon.especialDefense}/${pokemon.speed}/${pokemon.altura}/${pokemon.type}`">
                      <button id="poki">
                        ...
                        <span class="pword">poke</span>
                        <span class="pword2">Api</span>
                      </button>
                    </router-link>
      ...
      <div class="row">
        <div class="col-sm-2"></div>
        <div class="col-sm-8">
          <div class="d-grid gap-2 col-6 mx-auto" v-if="mostrar">

            <button class="btn btn-primary" @click="cargarMas()" type="button">Cargar Mas</button>
          </div>
        </div>
        <div class="col-sm-2"></div>
      </div>
...
const listaCompletaPokemon = ref([]);
const tiposPokemon = ref([]);
const tipoSeleccionado = ref('');
const pokemonLista = ref([]);
let inicio = 0;
const pokemonBuscado = ref('');
const mostrar = ref(true)

const Pokemon = async (inicio) => {
  try {
    const respuesta = await axios.get(`https://pokeapi.co/api/v2/pokemon?offset=${inicio}&limit=50`);
    ...
    const requests = results.map(async (result) => {
      ...
      return {
        ...
        power: pokemonData.base_experience,
        peso: pokemonData.weight,
        hp: pokemonData.stats[0].base_stat,
        attack: pokemonData.stats[1].base_stat,
        defense: pokemonData.stats[2].base_stat,
        especialAttack: pokemonData.stats[3].base_stat,
        especialDefense: pokemonData.stats[4].base_stat,
    ...
    pokemonLista.value = [...pokemonLista.value, ...pokemons];

    // Guardar la lista completa en listaCompletaPokemon
    listaCompletaPokemon.value = [...pokemonLista.value];
  } catch (error) {
    console.log(error);
  }
};
...
const obtenerTiposPokemon = async () => {
  try {
    const respuesta = await axios.get('https://pokeapi.co/api/v2/type');
    const results = respuesta.data.results;
    tiposPokemon.value = results.map((tipo) => tipo.name);
  } catch (error) {
    console.log(error);
  }
};
...
const filtrarPorTipo = () => {
  if (tipoSeleccionado.value === '') {
    // Restaurar la lista completa cuando se selecciona "Todos los tipos"
    pokemonLista.value = [...listaCompletaPokemon.value];
    mostrar.value = true;
  } else {
    const resultados = listaCompletaPokemon.value.filter(
      (pokemon) => pokemon.type === tipoSeleccionado.value
    );
    pokemonLista.value = resultados;
    mostrar.value = false;
  }
};

const buscarPokemon = () => {
  const nombreBuscado = pokemonBuscado.value.toLowerCase();
  if (nombreBuscado === '') {
    filtrarPorTipo(); // Filtrar en función del tipo seleccionado (si hay alguno)
    mostrar.value = true;
  } else {
    // Se filtra la lista completa de Pokémon (listaCompletaPokemon) en lugar de la lista actual (pokemonLista)
    const resultados = listaCompletaPokemon.value.filter(pokemon =>
